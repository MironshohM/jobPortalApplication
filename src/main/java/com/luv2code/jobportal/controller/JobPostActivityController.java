...
public class JobPostActivityController {
    ...
    @Autowired
    public JobPostActivityController(UsersService usersService, JobPostedActivityService jobPostedActivityService, JobSeekerApplyService jobSeekerApplyService, JobSeekerSaveService jobSeekerSaveService) {
    ...
    public String searchJobs(Model model, @RequestParam(value = "job",required = false)String job,
        ...
        if (!dateSearchFlag && !remote && !type && !StringUtils.hasText(job) && !StringUtils.hasText(location)) {
            jobPost = jobPostedActivityService.getAll();
        } else {
            jobPost = jobPostedActivityService.search(job, location, Arrays.asList(partTime, fullTime, freelance),
        ...
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            ...
            if(authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))){
                    List<RecruiterJobsDto> recruiterJobsDtos= jobPostedActivityService.getRecruiterJobs(((RecruiterProfile)currentUserProfile).getUserAccountId());
    ...
    public String addNew(JobPostActivity jobPostActivity,Model model){
        ...
        model.addAttribute("jobPostedActivity",jobPostActivity);
        JobPostActivity saved=jobPostedActivityService.addNew(jobPostActivity);
        return "redirect:/dashboard/";
    }

    @PostMapping("dashboard/edit/{id}")
    public String editJob(@PathVariable("id") int id,Model model){
        JobPostActivity  jobPostedActivity=jobPostedActivityService.getOne(id);

        model.addAttribute("jobPostedActivity",jobPostedActivity);
